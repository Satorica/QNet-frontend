      ...
      <div class="card-content">
        ...
        <div class="left-column graph-data-section">
          ...
          <div class="data-editor-grid" :style="{ '--matrix-columns': distanceMatrix.length }">
            ...
            <!-- TSP可视化 -->
            <div class="tsp-visualization graph-preview-panel">
              <div class="graph-preview-heading"><span>图形预览</span><small>城市与路径</small></div>
              <TSPGraph
                :cities="cities"
                :route="currentRoute"
                :bestRoute="bestRoute"
...
const submitSolve = async () => {
  ...
  try {
    ...
    resetSolveLogs(
      `开始求解旅行商问题（求解模型：${getModelTypeText(solveType.value)}，算法类型：${getMethodTypeText(methodType.value)}，${cityCount.value}个城市）`
    );
...
.title-bar {
  width: 4px;
  height: 28px;
  background: linear-gradient(180deg, var(--app-accent), var(--app-accent-secondary));
  border-radius: 2px;
}
