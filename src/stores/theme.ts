...
export const themes = [
  ...
  { id: 'sunset', name: '暖阳橙', primary: '#bd571e', secondary: '#dd8a30' },
...
function applyTheme(id: ThemeId) {
  ...
  // Authentication pages share the selected skin, including changes before logout.
  root.style.setProperty('--brand-primary', theme.primary)
  root.style.setProperty('--brand-rgb', rgb.join(', '))
  root.style.setProperty('--auth-background', mix(0.94, 0))
  for (const level of [1, 2, 3, 4, 5, 6, 7, 8, 9]) {
    root.style.setProperty(`--el-color-primary-light-${level}`, mix(level / 10))
    root.style.setProperty(`--brand-primary-light-${level}`, mix(level / 10))
  }
  root.style.setProperty('--el-color-primary-dark-2', mix(0.2, 0))
  root.style.setProperty('--brand-primary-dark-2', mix(0.2, 0))
